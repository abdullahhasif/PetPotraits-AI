          ...
          <div ref={scrollContainerRef} className="flex items-stretch gap-3 overflow-x-auto pb-2 -mb-2 snap-x snap-mandatory" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
            {SIZES.map((size) => (
              <button
                ...
                className={`flex-shrink-0 snap-start w-28 h-40 p-2 border-2 rounded-xl flex flex-col items-center justify-center text-center transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-pink-500
                  ${selectedSize.id === size.id ? 'border-pink-500 bg-pink-50' : 'border-zinc-200 bg-white hover:border-zinc-300 hover:bg-zinc-50'}`}
              >
                <SizeShapeIcon orientation={size.orientation} className={`w-10 h-10 mb-2 transition-colors ${selectedSize.id === size.id ? 'text-pink-500' : 'text-zinc-500'}`} />
                <span className={`font-semibold text-sm transition-colors ${selectedSize.id === size.id ? 'text-pink-600' : 'text-zinc-800'}`}>{size.label}</span>
